...
export class Address {
    ...
    public static parse(serialized: string): Address | undefined {
        if (serialized.endsWith(Server.User)) {
            const id = parseInt(serialized.substring(0, serialized.length - Server.User.length));
            return new UserAddress(id);
        } else if (serialized.endsWith(Server.Group)) {
            const id = parseInt(serialized.substring(0, serialized.length - Server.Group.length));
